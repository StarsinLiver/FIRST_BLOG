...
import CinemaService from '../../../services/normal/CinemaService';

function Cinema() {
    // 변수 정의
    const [cinema, setCinema] = useState<Array<ICinema>>([]); // 부서 배열
    const [searchQuestion, setSearchQuestion] = useState<string>(""); // 검색어 변수
    ...
    useEffect(() => {
      retrieveCinema(); // 전체 조회 함수 실행
    }, [page, pageSize]);
  
    //   TODO : 전체 조회 함수
    const retrieveCinema = () => {
      // 백엔드 매개변수 전송 : 검색어 LIKE 조회 + 현재페이지(page) , 1페이지당 개수(pageSize)
      CinemaService.getAll(searchQuestion, page - 1, pageSize)
        .then((response: any) => {
          // 백엔드 성공시 실행됨
          // es6(모던js) 문법 : 객체 분해 할당
          // const cinema = response.data.cinema;             // 부서 배열
          // const totalPages = response.data.totalPages; // 전체페이지수
          const { cinema, totalPages } = response.data;
          // cinema 저장
          setCinema(cinema);
          setCount(totalPages);
          // 로그 출력
    ...
    <div>
      ...
      <div className="col-md-12">
        <div className="accordion" id="accordionExample">
          {/* 1st 아코디언 시작 */}
          {/* 반복문 : cinema.map((data , index)=> (아코디언 태그)) */}
          {cinema &&
