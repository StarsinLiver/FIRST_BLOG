...
@SequenceGenerator(
        name = "SQ_BOARD_USER_GENERATOR"
        , sequenceName = "SQ_BOARD_USER"
        , initialValue = 1
        , allocationSize = 1
)
@NoArgsConstructor
...
@SQLDelete(sql = "UPDATE BOARD_USER SET DELETE_YN = 'Y', DELETE_TIME=TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') WHERE USER_ID = ?")

public class User extends BaseTimeEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE,
            generator = "SQ_BOARD_USER_GENERATOR")
    @Column(name = "USER_ID")
    ...
    private String descript; // Credential
    @Column(name = "ROLE")
